...
describe("ContactController", () => {
  ...
  describe('#addContact()', ()=> {
    it('should add a single contact into the book', (done) => {
      this.book.addContact("Alice", "001-101-1010", "alice@nowhere.com")
      .then((contact) => {
        expect(contact.name).toBe('Alice');
        expect(contact.phone).toBe('001-101-1010');
        expect(contact.email).toBe('alice@nowhere.com');
        done();
      })
      .catch((err) => {
        done();
  ...
  describe('#getContacts()', ()=> {
    ...
    it('should return an array of contacts when contacts are available', (done) => {
      this.book.addContact('Alice', '000-111-2222', 'alice@example.com')
      .then(() => {
        this.book.getContacts()
        .then((contacts) => {
  ...
  describe('search methods', () => {
    ...
    const alloy = ['Alloy Rodriguez', '111-111-111', 'alloy@guerilla-games.com'];

    describe('#iterativeSearch()', ()=> {
      it('should retun null when called with an empty array', () => {
        expect(this.book.iterativeSearch([], 'Alloy')).toBeNull();
      })

      it('should return null when target is not found', (done) => {
